...
void PX4CtrlFSM::process()
{
	...
	static bool start_once_land = false;
	bool kill_switch = false;
	std_msgs::Int32 fligh_mode_record;
	...
	case CMD_CTRL:
	{
		...
		else if (!rc_data.is_command_mode || !cmd_is_received(now_time))
		{
			...
			set_hov_with_odom();
			des = get_hover_des_add_v();
			ROS_INFO("\033[32m[px4ctrl] From CMD_CTRL(L3) --> AUTO_HOVER(L2)!\033[32m");
		}
		else
		{
			des = get_cmd_des();
	...
	case AUTO_TAKEOFF:
	{
		if(param.takeoff_land.height != takeoff_land_data.take_off_hight){	//为mission单独添加的起飞高度统一功能
			param.takeoff_land.height = takeoff_land_data.take_off_hight;
			ROS_INFO("\033[34m[px4ctrl] receive new takeoff Hight!!! %f\033[0m", takeoff_land_data.take_off_hight);
		}
	...
	case AUTO_LAND:
	{
		...
		else if (!rc_data.is_command_mode)
		{
			...
			set_hov_with_odom();
			des = get_hover_des_add_v();
			ROS_INFO("\033[32m[px4ctrl] From AUTO_LAND --> AUTO_HOVER(L2)!\033[32m");
		}
		else if (!get_landed())
		{
			if(!start_once_land){		//返回机巢用
				double distance_to_home_position = 
					sqrt(pow(takeoff_land.start_pose[0] - takeoff_land_data.home_position[0], 2) + 
						 pow(takeoff_land.start_pose[1] - takeoff_land_data.home_position[1], 2)  );
				if(distance_to_home_position < 0.2)
				{
					ROS_INFO("\033[34m[LAND AND TURN] go back home! change X = %f, Y = %f\033[0m", takeoff_land.start_pose[0], takeoff_land.start_pose[1]);
					takeoff_land.start_pose[0] = takeoff_land_data.home_position[0];
					takeoff_land.start_pose[1] = takeoff_land_data.home_position[1];
				}
				start_once_land = true;
			}
	...
	flight_mode_pub.publish(fligh_mode_record);
...
// 增加的代码 test
Desired_State_t PX4CtrlFSM::get_hover_des_add_v()
{
	Desired_State_t des;
	des.p = hover_pose.head<3>();
	des.v = hover_vel;  // 增加速度控制
	des.a = Eigen::Vector3d::Zero();
	des.j = Eigen::Vector3d::Zero();
	des.yaw = hover_pose(3);
	des.yaw_rate = 0.0;
...
void PX4CtrlFSM::set_hov_with_rc()
{
	...
	double cos_yaw = qw*qw + qx*qx - qy*qy - qz*qz;
	double sin_yaw = 2.0 * (qx*qy + qw*qz);

	// 局部坐标系速度
	Eigen::Vector3d vel_from_rc;
	vel_from_rc(0) = rc_data.ch[1] * param.max_manual_vel * (param.rc_reverse.pitch ? 1 : -1);		// X
	vel_from_rc(1) = rc_data.ch[0] * param.max_manual_vel * (param.rc_reverse.roll ? 1 : -1);		// Y
	vel_from_rc(2) = rc_data.ch[2] * param.max_manual_vel * (param.rc_reverse.throttle ? 1 : -1);	// Z

	//局部坐标系转全局
	hover_vel(0) = vel_from_rc(0) * cos_yaw - vel_from_rc(1) * sin_yaw;  // 全局x速度
	hover_vel(1) = vel_from_rc(0) * sin_yaw + vel_from_rc(1) * cos_yaw;  // 全局y速度
	hover_vel(2) = vel_from_rc(2);  // z轴速度不变，直接赋值

	// 位置积分
	hover_pose.head<3>() += hover_vel * delta_t;
	
	// yaw
	hover_pose(3) += rc_data.ch[3] * param.max_manual_vel * delta_t * (param.rc_reverse.yaw ? 1 : -1);
	if (hover_pose(3) > M_PI)		//yaw标准化
		hover_pose(3) -= 2 * M_PI;
	else if (hover_pose(3) <= -M_PI)
		hover_pose(3) += 2 * M_PI;
