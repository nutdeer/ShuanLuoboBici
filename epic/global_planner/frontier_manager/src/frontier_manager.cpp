...
// 根据激光雷达深度图 判断是否存在间隙点 (通过深度连续性+邻域扩散实现)
...
// 前沿提取 (原始点云->未知边界集合) (核心函数) (主接口)
// 数据预处理→网格筛选→边界分类→法向量验证→聚类更新
void FrontierManager::updateFrontierClusters(
  ...
  */
  ...
  // 去噪步骤(真实边界的法向量应指向 “未知区域”（无障碍物），反方向指向 “已知区域”（有障碍物）)
  omp_set_num_threads(4);
  // clang-format off
  #pragma omp parallel for
  // clang-format on
...
void FrontierManager::selectBestViewpoint(ClusterInfo::Ptr &cluster) {
  ...
  for (int i = 0; i < vps.size(); i++) {
    ...
    // }
    yaw[i] = (45.0 * (max_yaw_idx - 4) + 22.5) / 180.0 * M_PI;  //最优朝向
  }
...
// 视点初始化 通过DBSCAN创建候选视点
// 使用预定义视点模板生成候选视点->DBscan基于联通性聚类->生成最后的视点聚类表
...
// 移除不可达的视点聚类(临时插入拓扑图 删除不可达节点)
void FrontierManager::removeUnreachableViewpoints(vector<ClusterInfo::Ptr> &clusters) 
{
...
// 球面坐标转2D网格索引
inline void FrontierManager::Sphere_PosToIndex(const Eigen::Vector3f &lidar_center,
