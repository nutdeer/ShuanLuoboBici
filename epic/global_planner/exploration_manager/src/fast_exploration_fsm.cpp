...
{
  // **其他状态下 只进行前沿可视化**
  if (!(state_ == WAIT_TRIGGER || state_ == PLAN_TRAJ || state_ == EXEC_TRAJ || state_ == FINISH))
  {
    global_path_update_timer_.stop();
  ...
  planner_manager_->topo_graph_->getRegionsToUpdate();    //获取拓扑图需要更新的区域
  // cout << "getRegionsToUpdate time cost:" << (ros::Time::now() - t2).toSec() * 1000 << "ms" << endl;
  planner_manager_->topo_graph_->updateSkeleton();        //更新拓扑图

  ros::Time t3 = ros::Time::now();
  planner_manager_->topo_graph_->updateOdomNode(fd_->odom_pos_, fd_->odom_yaw_);
  planner_manager_->topo_graph_->updateHistoricalOdoms(); //更新历史轨迹

  // **碰撞检查**
  if (planner_manager_->topo_graph_->odom_node_->neighbors_.empty())  // 拓扑图安全检查
  {
    double time;
  ...
  if (planner_manager_->local_data_.traj_id_ > 1)  // 轨迹安全性检查
  {
  ...
  int res = expl_manager_->planGlobalPath(odom, vel);   // 探索路径规划 (筛选点+求解TSP)
