...
void FastExplorationManager::initialize(
  ...
  frontier_manager_ptr_ = frt_manager;  //边界搜索管理器(生成探索目标点)
  planner_manager_ = planner_manager;   //局部规划管理器(生成拓扑图 路径搜索)

  ed_.reset(new ExplorationData);   //储存探索状态
  ep_.reset(new ExplorationParam);  //储存探索参数
                                           ...
                                           TopoNode::Ptr &n2, float &yaw2)
{
                          ...
                          vector<Eigen::Vector3f> &path) -> double
  {
    ...
    if (res == BubbleAstar::NO_PATH)
      return 2e3 + (n1->center_ - n2->center_).norm(); // 使用一个大的时间值表示无法到达
    if (res == BubbleAstar::START_FAIL || res == BubbleAstar::END_FAIL)
      return 2e3 + (n1->center_ - n2->center_).norm(); // 同上，用于不同的错误情况
...
// 路径代价计算 直接使用原始成本 A*搜索一个路 再计算距离成本
...
// 全局探索路径规划 (核心函数)
// 生成探索目标点→筛选可达目标→构建 TSP 成本矩阵→求解 TSP 得到最优探索顺序→生成全局路径
